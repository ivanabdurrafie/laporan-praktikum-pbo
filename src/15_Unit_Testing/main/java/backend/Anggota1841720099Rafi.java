...
public class Anggota1841720099Rafi {
    ...
    ArrayList<Anggota1841720099Rafi> getByNamaAlamatAndTeleponRafi(String mNamaRafi, String mAlamatRafi, String mTeleponRafi) {
         ArrayList<Anggota1841720099Rafi> ListKategori = new ArrayList<>();
        ResultSet rs;

        if ((mNama.trim().length() > 0) && (mAlamat.trim().length() > 0)) {
            rs = DBHelper1841720099Rafi.selectQueryRafi("SELECT * FROM anggota Where nama = '"
                    + mNama + "' and alamat = '" + mAlamat + "' and telepon = '" + mTelepon + "'");
        } else if ((mNama.trim().length() > 0) && (mAlamat.trim().length() == 0) && (mTelepon.trim().length() == 0)) {
            rs = DBHelper1841720099Rafi.selectQueryRafi("SELECT * FROM anggota Where nama = '"
                    + mNama + "'");
        } else if ((mNama.trim().length() == 0) && (mAlamat.trim().length() == 0) && (mTelepon.trim().length() > 0)) {
            rs = DBHelper1841720099Rafi.selectQueryRafi("SELECT * FROM anggota Where telepon = '"
                    + mTelepon + "'");
        } else {
            rs = DBHelper1841720099Rafi.selectQueryRafi("SELECT * FROM anggota Where alamat'" + mAlamat + "'  ");
        }

        try {
            while (rs.next()) {
                Anggota1841720099Rafi agt = new Anggota1841720099Rafi();
                agt.setmIdAnggotaRafi(rs.getInt("idAnggota"));
                agt.setmNamaRafi(rs.getString("nama"));
                agt.setmAlamatRafi(rs.getString("alamat"));
                agt.setmTeleponRafi(rs.getString("telepon"));
